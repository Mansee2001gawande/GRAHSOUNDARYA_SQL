...
export const getDesignerByCity = (request,response,next)=>{
    let city = request.body.city;

    Professional.getDesignerByCity(city)
    .then(result => {
        console.log(result)
        if (result.length != 0)
...
export const reviewOnDesign = (request,response,next)=>{
    let design_id = request.body.design_id;
    Professional.reviewOnDesign(design_id)
    .then(result=>{
        if (result.length != 0)
            return response.status(200).json({ data: result });
        return response.status(404).json("Sorry, we couldn't find the information");
...
//see image on chrome http://localhost:3000/images/33b6735f2735a7f24dd4f71f8e71fcd5
// after download random no write .png
